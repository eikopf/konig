...
fn get_piece_from_into_board() {
    let index = StandardIndex::try_from("e1").unwrap(); // should be 4
...
fn get_piece_from_standard_board() {
    let index = StandardIndex::try_from("h1").unwrap();
...
fn index_fen_board_with_standard_index() {
    ...
    let index = StandardIndex::try_from("b5").unwrap(); // 33
    assert_eq!(fen_board[index], Some(StandardPiece::WhiteBishop));

    let index = StandardIndex::try_from("a5").unwrap(); // 32
    assert_eq!(fen_board[index], Some(StandardPiece::BlackPawn));

    let index = StandardIndex::try_from("b6").unwrap(); // 41
    assert_eq!(fen_board[index], None);

    let index = StandardIndex::try_from("h3").unwrap(); // 23
